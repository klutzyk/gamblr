...
def compute_prediction_features(
    ...
    team_game = (
        df_history.groupby(["game_id", "team_abbreviation", "game_date"], as_index=False)[
            "points"
        ]
        .sum()
        .rename(columns={"points": "team_points"})
    ...
    team_features = team_game.groupby("team_abbreviation").apply(
        lambda x: pd.Series(
            {
                "team_points_avg_last5": x["team_points"].tail(5).mean(),
                "team_points_avg_last10": x["team_points"].tail(10).mean(),
                "opponent_points_allowed_last5": x["opponent_points"].tail(5).mean(),
            }
        )
    )
