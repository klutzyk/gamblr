...
function formatDateByRegion(dateString: string | null | undefined, regionConfig: (typeof REGION_CONFIG)[UserRegion]) {
...
export default function PerformancePage() {
  ...
  useEffect(() => {
    void Promise.all([
      ...
      safeLoad(setRecentState, () => getReviewRecent({ stat_type: statType, days: Math.min(days, 30), limit: 24 })),
    ]).catch(() => undefined);
  ...
  return (
    <div className="perf-shell">
      <div className="perf-background" />
      <header className="perf-topbar">
        <div className="perf-brand">
          <img src={logo} alt="Gamblr" className="perf-logo" />
          <div>
            <p className="perf-kicker">Gamblr</p>
            <h1>How We&apos;re Doing</h1>
          </div>
        </div>
        <div className="perf-topbar-actions">
          <a className="perf-link-btn" href="/">
            Back to main app
          </a>
        </div>
      </header>
      ...
      <main className="perf-main">
        <section className="perf-hero">
          ...
          <div className="perf-hero-controls">
            ...
            <label>
              Region
              <select value={userRegion} onChange={(e) => setUserRegion(e.target.value as UserRegion)}>
        ...
        <section className="perf-card-grid">
          <article className="perf-stat-card">
          ...
          <article className="perf-stat-card">
            ...
            <p>Lower is better. This is the average distance from the actual result.</p>
          </article>
          <article className="perf-stat-card">
            <span className="perf-card-label">Typical miss</span>
            <strong>{formatNumber(overviewState.data?.median_miss)} {statMeta.unit}</strong>
            <p>This shows the more typical miss, without being skewed by outliers.</p>
          </article>
          <article className="perf-stat-card">
            <span className="perf-card-label">Close-call rate</span>
            <strong>{formatPercent(overviewState.data?.close_rate)}</strong>
            <p>Share of tracked picks that finished within a tight margin.</p>
          </article>
          <article className="perf-stat-card">
            <span className="perf-card-label">Tends to run</span>
            <strong>{overviewState.data?.bias_label ?? "-"}</strong>
            <p>
              {typeof overviewState.data?.bias === "number"
                ? `${Math.abs(overviewState.data.bias).toFixed(1)} ${statMeta.unit} on average`
                : "Not enough completed data yet"}
            </p>
          </article>
          <article className="perf-stat-card perf-stat-card-accent">
        ...
        <section className="perf-two-col">
          ...
          <article className="perf-panel perf-panel-story">
            <div className="perf-panel-head">
              <div>
                <p className="perf-kicker">Plain-English read</p>
            ...
            <div className="perf-story-list">
              ...
              <div className="perf-story-pill">
                ...
                <strong>
                  {strongestPlayer
                    ? `${strongestPlayer.full_name} has been one of the steadier ${statMeta.label.toLowerCase()} reads.`
              ...
              <div className="perf-story-pill">
                ...
                <strong>
                  {shakiestPlayer
                    ? `${shakiestPlayer.full_name} has been more volatile lately, so treat that read with extra caution.`
          ...
          <article className="perf-panel perf-player-focus">
            <div className="perf-panel-head">
              ...
              <span className="perf-panel-hint">
                {selectedPlayer ? `${selectedPlayer.team_abbreviation} · ${selectedPlayer.reliability_tag}` : "Player detail will appear here."}
              </span>
            </div>
              ...
              <>
                ...
                <div className="perf-mini-cards">
                  ...
                  <div>
                    ...
                    <strong>{playerDetailState.data.bias && playerDetailState.data.bias > 0 ? "High" : playerDetailState.data.bias && playerDetailState.data.bias < 0 ? "Low" : "Balanced"}</strong>
                  </div>
                </div>
